...
//generateSphere(200, 1.0, &(_vertexData), &(_texCoords), &_indices, &_numVetex);
int generateSphere(int numSlices, float radius, float **vertices,
                float **texCoords, uint16_t **indices, int *numVertices_out) {
    int i;
    int j;
    int numParallels = numSlices / 2;//200/2
    int numVertices = (numParallels + 1) * (numSlices + 1);//(100+1)*(200+1)
    int numIndices = numParallels * numSlices * 6;//100*200*6
    float angleStep = (2.0f * ES_PI) / ((float) numSlices);//2*3.1415926/200
